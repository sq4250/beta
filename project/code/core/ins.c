...
static f32 g_theta      = 0.0f;  // 融合航向 [rad]
static f32 g_theta_rate = 0.0f;  // 陀螺 z 轴 [rad/s]
...
void ins_fuse_theta(f32 gyro_z, const f32 quat[4], bool has_quat) {
    ...
    if (has_quat) {
        ...
        g_theta = IMU_YAW_ALPHA * (g_theta + g_theta_rate * ISR_DT) + IMU_QUAT_ALPHA * yq;
    } else {
        g_theta += g_theta_rate * ISR_DT;
    }
}
