/**
 * model_factory.h — 模型工厂: 统一接口 + 自描述物理约束
 *
 * 每个模型 .c 提供 const model_desc_t 实例, 包含:
 *   forward      — 模型入口函数指针
 *   a_long/brake/lat/v — 物理约束
 *
 * 混合模式: planner_forward_hybrid 按目标类型切换 g_model_active
 * 单模型:   g_model_active 始终指向所选模型
 */
...
/* ── 当前活跃模型 (planner 设置, kinematics 读取) ── */
