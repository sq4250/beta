...
bool hal_imu_has_quat(ConstImuHandle h)         { return h->drv->has_quat(); }
void hal_imu_read_quat(f32 q[4], ConstImuHandle h) { h->drv->read_quat(q); }
f32  hal_imu_gyro_bias_z(ConstImuHandle h)         { return h->bias_gz; }
