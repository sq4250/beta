/**
 * utils.c — 通用工具函数
 */
#include "utils.h"

f32 wrap_pi(f32 rad) {
    while (rad >  M_PI_F) rad -= 2.0f * M_PI_F;
    while (rad < -M_PI_F) rad += 2.0f * M_PI_F;
    return rad;
}

f32 bicycle_curvature(f32 v, f32 delta) {
    return v * tanf(delta) * INV_WHEELBASE;
}

bool check_hit_substep(const Waypoint *prev, const Waypoint *next, const Waypoint *target, f32 tol) {
    f32 bx = next->x, by = next->y;
    f32 px = target->x, py = target->y;
    if ((bx-px)*(bx-px) + (by-py)*(by-py) < tol*tol) return true;

    f32 abx = bx - prev->x, aby = by - prev->y;
    f32 ab2 = abx*abx + aby*aby;
    if (ab2 < 1e-12f) return false;

    f32 t = ((px-prev->x)*abx + (py-prev->y)*aby) / ab2;
    t = clamp(t, 0.0f, 1.0f);
    f32 cx = prev->x + t*abx, cy = prev->y + t*aby;
    return (px-cx)*(px-cx) + (py-cy)*(py-cy) < tol*tol;
}
